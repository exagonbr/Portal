...
class TargetAudienceController extends BaseController<TargetAudience> {
 ...
 async toggleStatus(req: Request, res: Response) {
  try {
   const { id } = req.params;
   const targetAudience = await this.targetAudienceRepository.toggleStatus(id);
   if (!targetAudience) {
 ...
 async search(req: Request, res: Response) {
  try {
   ...
   const targetAudiences = await this.targetAudienceRepository.findByName(q as string);
 ...
 async getActive(req: Request, res: Response) {
  try {
   const targetAudiences = await this.targetAudienceRepository.findActive();
   return res.status(200).json({ success: true, data: targetAudiences });
  } catch (error) {
