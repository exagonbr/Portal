...
class EducationPeriodController extends BaseController<EducationPeriod> {
  ...
  async toggleStatus(req: Request, res: Response) {
    try {
      const { id } = req.params;
      const educationPeriod = await this.educationPeriodRepository.toggleStatus(id);
      if (!educationPeriod) {
  ...
  async search(req: Request, res: Response) {
    try {
      ...
      const educationPeriods = await this.educationPeriodRepository.findByDescription(q as string);
  ...
  async getActive(req: Request, res: Response) {
    try {
      const educationPeriods = await this.educationPeriodRepository.findActive();
      return res.status(200).json({ success: true, data: educationPeriods });
    } catch (error) {
