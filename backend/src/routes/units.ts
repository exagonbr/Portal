...
router.get('/', authMiddleware, async (req, res) => {
  try {
    ...
    return res.json(createPaginatedResponse(formattedUnits, {
...
router.get('/search', authMiddleware, async (req, res) => {
  try {
    ...
    return res.json(createApiResponse(true, formattedUnits));
  } catch (error) {
...
router.get('/:id', authMiddleware, async (req, res) => {
  try {
    ...
    if (!unit) {
      return res.status(404).json(createApiResponse(false, undefined, undefined, 'Unit not found'));
    }

    const formattedUnit = formatUnitResponse(unit);

    return res.json(createApiResponse(true, formattedUnit));
  } catch (error) {
...
router.post('/', authMiddleware, async (req, res) => {
  try {
    ...
    if (!name || !institution_id) {
      return res.status(400).json(createApiResponse(false, undefined, undefined, 'Name and institution_id are required'));
    }

    // If user has institution_id, enforce it
    if ((req.user as any)?.institutionId && institution_id !== (req.user as any)?.institutionId) {
      return res.status(403).json(createApiResponse(false, undefined, undefined, 'Cannot create unit for different institution'));
    }
    ...
    return res.status(201).json(createApiResponse(true, formattedUnit, 'Unit created successfully'));
  } catch (error) {
...
router.put('/:id', authMiddleware, async (req, res) => {
  try {
    ...
    return res.json(createApiResponse(true, formattedUnit, 'Unit updated successfully'));
  } catch (error) {
...
router.delete('/:id', authMiddleware, async (req, res) => {
  try {
    ...
    return res.json(createApiResponse(true, undefined, 'Unit deleted successfully'));
  } catch (error) {
