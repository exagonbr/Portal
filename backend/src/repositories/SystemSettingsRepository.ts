...
export class SystemSettingsRepository extends BaseRepository {
  ...
  async findAllPaginated(options: {
    ...
    limit?: number;
    search?: string;
  } = {}): Promise<PaginatedResult<SystemSettings>> {
    const { page = 1, limit = 10, search } = options;
    
    try {
      if (this.repository) {
        let queryBuilder = this.repository.createQueryBuilder('systemsettings');
        
        // Adicione condições de pesquisa específicas para esta entidade
        if (search) {
          queryBuilder = queryBuilder
            .where('systemsettings.name ILIKE :search', { search: `%${search}%` });
        }
      ...
      console.error(`Erro ao buscar registros de systemsettings:`, error);
      throw error;
    }
  }
  ...
  async findAll(options: {
    ...
    try {
      ...
      console.error(`Erro ao buscar registros de system_settings:`, error);
      throw error;
    }
  }
  ...
  async findById(id: number): Promise<any | null> {
    try {
      ...
      console.error(`Erro ao buscar registro por ID em system_settings:`, error);
      throw error;
    }
  }
  ...
  async create(data: any): Promise<any> {
    try {
      ...
      const query = `
        INSERT INTO system_settings (${fields.join(', ')})
        VALUES (${placeholders})
        RETURNING *
      `;
      ...
      console.error(`Erro ao criar registro em system_settings:`, error);
      throw error;
    }
  }
  ...
  async update(id: number, data: any): Promise<any | null> {
    try {
      ...
      console.error(`Erro ao atualizar registro em system_settings:`, error);
      throw error;
    }
  }
  ...
  async delete(id: number): Promise<boolean> {
    try {
      ...
      console.error(`Erro ao deletar registro em system_settings:`, error);
      throw error;
    }
  }
}
