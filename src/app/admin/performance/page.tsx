'use client'

import { useAuth } from '@/contexts/AuthContext'
import { useState } from 'react'
import DashboardLayout from '@/components/dashboard/DashboardLayout'
import DashboardPageLayout from '@/components/dashboard/DashboardPageLayout'
import ProtectedRoute from '@/components/auth/ProtectedRoute'
import { UserRole } from '@/types/roles'

export default function AdminPerformancePage() {
  const { user } = useAuth()
  const [selectedPeriod, setSelectedPeriod] = useState('7d')

  const performanceData = {
    overview: {
      totalRequests: 1245876,
      avgResponseTime: 124,
      errorRate: 0.15,
      uptime: 99.97
    },
    metrics: [
      { name: 'Requisições/min', value: 2847, change: 12.5, trend: 'up' },
      { name: 'Tempo de Resposta', value: 124, change: -8.2, trend: 'down' },
      { name: 'Taxa de Erro', value: 0.15, change: -0.05, trend: 'down' },
      { name: 'Uso de CPU', value: 68, change: 5.3, trend: 'up' },
      { name: 'Uso de Memória', value: 72, change: 2.1, trend: 'up' },
      { name: 'Throughput', value: 156, change: 18.7, trend: 'up' }
    ],
    endpoints: [
      { path: '/api/users', requests: 45678, avgTime: 89, errorRate: 0.12 },
      { path: '/api/courses', requests: 34521, avgTime: 156, errorRate: 0.08 },
      { path: '/api/auth/login', requests: 28945, avgTime: 234, errorRate: 0.25 },
      { path: '/api/content', requests: 23456, avgTime: 312, errorRate: 0.45 },
      { path: '/api/files', requests: 18734, avgTime: 445, errorRate: 0.89 }
    ]
  }

  return (
    <ProtectedRoute requiredRole={[UserRole.SYSTEM_ADMIN]}>
      <DashboardLayout>
        <DashboardPageLayout
          title="Performance do Sistema"
          subtitle="Análise detalhada da performance e otimização"
        >
          <div className="space-y-6">
            {/* Controles */}
            <div className="flex justify-between items-center">
              <div className="flex space-x-4">
                <select 
                  value={selectedPeriod}
                  onChange={(e) => setSelectedPeriod(e.target.value)}
                  className="px-4 py-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-primary"
                >
                  <option value="1h">Última hora</option>
                  <option value="24h">Últimas 24 horas</option>
                  <option value="7d">Últimos 7 dias</option>
                  <option value="30d">Últimos 30 dias</option>
                </select>
              </div>
              <div className="flex space-x-3">
                <button className="bg-gray-100 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-200 transition-colors duration-200">
                  Exportar Relatório
                </button>
                <button className="bg-primary text-white px-4 py-2 rounded-lg hover:bg-primary/80 transition-colors duration-200">
                  Atualizar
                </button>
              </div>
            </div>

            {/* Métricas Principais */}
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              <div className="bg-white rounded-lg shadow-md p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-sm font-medium text-gray-500 mb-1">Total de Requisições</div>
                    <div className="text-2xl font-bold text-gray-600">
                      {performanceData.overview.totalRequests.toLocaleString()}
                    </div>
                  </div>
                  <div className="p-3 rounded-full bg-primary/20">
                    <span className="material-symbols-outlined text-lg text-primary">analytics</span>
                  </div>
                </div>
                <div className="mt-4 flex items-center">
                  <span className="text-accent-green text-sm">↑ 12.5%</span>
                  <span className="text-gray-500 text-sm ml-2">vs. período anterior</span>
                </div>
              </div>

              <div className="bg-white rounded-lg shadow-md p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-sm font-medium text-gray-500 mb-1">Tempo de Resposta</div>
                    <div className="text-2xl font-bold text-gray-600">
                      {performanceData.overview.avgResponseTime}ms
                    </div>
                  </div>
                  <div className="p-3 rounded-full bg-accent-blue/20">
                    <span className="material-symbols-outlined text-lg text-accent-blue">speed</span>
                  </div>
                </div>
                <div className="mt-4 flex items-center">
                  <span className="text-accent-green text-sm">↓ 8.2ms</span>
                  <span className="text-gray-500 text-sm ml-2">vs. período anterior</span>
                </div>
              </div>

              <div className="bg-white rounded-lg shadow-md p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-sm font-medium text-gray-500 mb-1">Taxa de Erro</div>
                    <div className="text-2xl font-bold text-gray-600">
                      {performanceData.overview.errorRate}%
                    </div>
                  </div>
                  <div className="p-3 rounded-full bg-accent-green/20">
                    <span className="material-symbols-outlined text-lg text-accent-green">check_circle</span>
                  </div>
                </div>
                <div className="mt-4 flex items-center">
                  <span className="text-accent-green text-sm">↓ 0.05%</span>
                  <span className="text-gray-500 text-sm ml-2">vs. período anterior</span>
                </div>
              </div>

              <div className="bg-white rounded-lg shadow-md p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-sm font-medium text-gray-500 mb-1">Uptime</div>
                    <div className="text-2xl font-bold text-gray-600">
                      {performanceData.overview.uptime}%
                    </div>
                  </div>
                  <div className="p-3 rounded-full bg-accent-green/20">
                    <span className="material-symbols-outlined text-lg text-accent-green">trending_up</span>
                  </div>
                </div>
                <div className="mt-4 flex items-center">
                  <span className="text-accent-green text-sm">↑ 0.03%</span>
                  <span className="text-gray-500 text-sm ml-2">vs. período anterior</span>
                </div>
              </div>
            </div>

            {/* Métricas Detalhadas */}
            <div className="bg-white rounded-lg shadow-md">
              <div className="px-6 py-4 border-b border-gray-200">
                <h3 className="text-lg font-medium text-gray-600">Métricas Detalhadas</h3>
              </div>
              <div className="p-6">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {performanceData.metrics.map((metric, index) => (
                    <div key={index} className="border border-gray-200 rounded-lg p-4">
                      <div className="flex items-center justify-between mb-3">
                        <h4 className="font-medium text-gray-700">{metric.name}</h4>
                        <span className={`
                          flex items-center text-sm
                          ${metric.trend === 'up' 
                            ? metric.name.includes('Erro') ? 'text-error' : 'text-accent-green'
                            : 'text-accent-green'}
                        `}>
                          <span className="material-symbols-outlined text-xs mr-1">
                            {metric.trend === 'up' ? 'trending_up' : 'trending_down'}
                          </span>
                          {Math.abs(metric.change)}%
                        </span>
                      </div>
                      <div className="text-2xl font-bold text-gray-600 mb-2">
                        {metric.value}
                        {metric.name.includes('Taxa') ? '%' : 
                         metric.name.includes('Tempo') ? 'ms' : 
                         metric.name.includes('Throughput') ? 'MB/s' : ''}
                      </div>
                      <div className="w-full bg-gray-200 rounded-full h-2">
                        <div 
                          className={`h-2 rounded-full ${
                            metric.name.includes('Erro') && metric.value > 1 ? 'bg-error' :
                            metric.value > 80 ? 'bg-accent-yellow' : 'bg-accent-green'
                          }`}
                          style={{ 
                            width: `${metric.name.includes('Taxa') ? metric.value * 20 : 
                                      metric.name.includes('Tempo') ? Math.min(metric.value / 5, 100) :
                                      metric.value}%` 
                          }}
                        />
                      </div>
                    </div>
                  ))}
                </div>
              </div>
            </div>

            {/* Análise de Endpoints */}
            <div className="bg-white rounded-lg shadow-md">
              <div className="px-6 py-4 border-b border-gray-200">
                <h3 className="text-lg font-medium text-gray-600">Performance por Endpoint</h3>
              </div>
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">
                        Endpoint
                      </th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">
                        Requisições
                      </th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">
                        Tempo Médio
                      </th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">
                        Taxa de Erro
                      </th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">
                        Status
                      </th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">
                        Ações
                      </th>
                    </tr>
                  </thead>
                  <tbody className="bg-white divide-y divide-gray-200">
                    {performanceData.endpoints.map((endpoint, index) => (
                      <tr key={index}>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <code className="text-sm bg-gray-100 px-2 py-1 rounded">{endpoint.path}</code>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                          {endpoint.requests.toLocaleString()}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                          <span className={`${
                            endpoint.avgTime > 400 ? 'text-error' :
                            endpoint.avgTime > 200 ? 'text-accent-yellow' : 'text-accent-green'
                          }`}>
                            {endpoint.avgTime}ms
                          </span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm">
                          <span className={`${
                            endpoint.errorRate > 0.5 ? 'text-error' :
                            endpoint.errorRate > 0.2 ? 'text-accent-yellow' : 'text-accent-green'
                          }`}>
                            {endpoint.errorRate}%
                          </span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className={`
                            px-2 inline-flex text-xs leading-5 font-semibold rounded-full
                            ${endpoint.errorRate > 0.5 ? 'bg-error/20 text-error' :
                              endpoint.avgTime > 400 ? 'bg-accent-yellow/20 text-accent-yellow' :
                              'bg-accent-green/20 text-accent-green'}
                          `}>
                            {endpoint.errorRate > 0.5 ? 'Crítico' :
                             endpoint.avgTime > 400 ? 'Lento' : 'Normal'}
                          </span>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                          <button className="text-primary hover:text-primary/80 mr-3 transition-colors duration-200">
                            Analisar
                          </button>
                          <button className="text-accent-blue hover:text-accent-blue/80 transition-colors duration-200">
                            Otimizar
                          </button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              </div>
            </div>

            {/* Recomendações de Otimização */}
            <div className="bg-white rounded-lg shadow-md">
              <div className="px-6 py-4 border-b border-gray-200">
                <h3 className="text-lg font-medium text-gray-600">Recomendações de Otimização</h3>
              </div>
              <div className="p-6">
                <div className="space-y-4">
                  <div className="border-l-4 border-accent-blue bg-accent-blue/10 p-4 rounded">
                    <div className="flex items-start">
                      <span className="material-symbols-outlined text-accent-blue mr-3 mt-1">lightbulb</span>
                      <div>
                        <div className="font-medium text-accent-blue">Otimizar endpoint /api/content</div>
                        <div className="text-sm text-gray-600 mt-1">
                          Este endpoint tem tempo de resposta alto (312ms). Considere implementar cache ou otimizar queries.
                        </div>
                      </div>
                    </div>
                  </div>
                  
                  <div className="border-l-4 border-accent-yellow bg-accent-yellow/10 p-4 rounded">
                    <div className="flex items-start">
                      <span className="material-symbols-outlined text-accent-yellow mr-3 mt-1">warning</span>
                      <div>
                        <div className="font-medium text-accent-yellow">Monitorar endpoint /api/files</div>
                        <div className="text-sm text-gray-600 mt-1">
                          Taxa de erro elevada (0.89%). Verificar logs para identificar problemas.
                        </div>
                      </div>
                    </div>
                  </div>
                  
                  <div className="border-l-4 border-accent-green bg-accent-green/10 p-4 rounded">
                    <div className="flex items-start">
                      <span className="material-symbols-outlined text-accent-green mr-3 mt-1">check_circle</span>
                      <div>
                        <div className="font-medium text-accent-green">Performance geral estável</div>
                        <div className="text-sm text-gray-600 mt-1">
                          O sistema está operando dentro dos parâmetros normais na maioria dos endpoints.
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </DashboardPageLayout>
      </DashboardLayout>
    </ProtectedRoute>
  )
}