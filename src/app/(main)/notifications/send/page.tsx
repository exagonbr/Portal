  ...
  useEffect(() => {
    const loadAvailableUsers = async () => {
      try {
        ...
        if (error?.status === 401) {
          console.error('🔐 [NOTIFICATIONS] Erro de autenticação ao carregar usuários')
          clearAllTokens()
        }
  ...
  const handleSendEmail = async (emailData: EmailData) => {
    ...
    try {
      ...
      if (error?.message?.includes('401') || error?.status === 401) {
        console.error('🔐 [NOTIFICATIONS] Erro de autenticação detectado')
        clearAllTokens()
    ...
    return (
      <div className="container-responsive spacing-y-responsive">
        <div className="text-center py-12">
          ...
          <p className="text-gray-500 mb-6">Você não tem permissão para enviar notificações.</p>
          <button
