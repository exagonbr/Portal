...
export default function SessionVideoPlayer({ 
  ...
  const currentVideo = sessionVideos[currentVideoIndex];
  ...
  const handleNextVideo = () => {
    if (currentVideoIndex < sessionVideos.length - 1) {
      handleVideoSelect(currentVideoIndex + 1);
    }
  };
  ...
  return createPortal(
    <div
      className="fixed inset-0 z-[99999] flex bg-black backdrop-blur-md"
      ...
      <div className="flex w-full h-full">
        ...
        <div className="w-80 bg-gray-900 flex flex-col border-r border-gray-700">
          ...
          <div className="p-6 border-b border-gray-700">
            <h2 className="text-xl font-bold text-white mb-2">{collectionName}</h2>
            <p className="text-gray-400 text-sm">Sessão {sessionNumber} • {sessionVideos.length} vídeos</p>
          </div>
        ...
        <div className="flex-1 flex flex-col">
          ...
          <div className="flex-1 relative bg-black">
            ...
            <iframe
              ...
              src={`https://www.youtube.com/embed/${getYouTubeId(currentVideo.video_url || '')}?autoplay=1&rel=0&modestbranding=1&playsinline=1`}
            ...
            {/* Navigation controls */}
            <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex items-center gap-4 bg-black bg-opacity-70 rounded-lg px-4 py-2">
              <button
              ...
              <span className="text-white text-sm">
                {currentVideoIndex + 1} de {sessionVideos.length}
              </span>
              
              <button
                onClick={handleNextVideo}
                disabled={currentVideoIndex === sessionVideos.length - 1}
                className="flex items-center gap-2 px-3 py-1 text-white disabled:text-gray-500 disabled:cursor-not-allowed hover:bg-white hover:bg-opacity-20 rounded transition-colors"
              >
                Próximo
            ...
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                <p className="text-gray-400 text-sm mb-2">
                  Episódio {currentVideo.episode_number || currentVideoIndex + 1} • Sessão {sessionNumber}
                </p>
