      ...
      <div className="bg-white dark:bg-gray-50 rounded-lg p-6 w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto">
          {(activeTab === 'assign' || manager) ? (
            ...
            <div>
              {managers.length === 0 ? (
                <p className="text-center text-gray-500 py-8">Nenhum gestor vinculado a esta escola</p>
              ) : (
                  ...
                  {managers.map((manager) => (
                    <div
                      ...
                      className={`flex items-center justify-between p-4 rounded-lg ${
                        manager.is_active ? 'bg-gray-50 dark:bg-gray-100' : 'bg-gray-100 dark:bg-gray-200 opacity-75'
                      }`}
                    >
                      <div>
                        <p className="font-medium">{manager.user_name || 'Usuário não encontrado'}</p>
