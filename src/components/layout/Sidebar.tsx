...
const NavItem = memo(({ item, isActive, isCollapsed, onClick, theme, isSystemAdmin = false }: {
  ...
  return (
    ...
    >
      <button
        onClick={handleClick}
        className={`flex items-center gap-2 px-2 transition-all duration-200 rounded-md mx-1 text-xs font-medium group relative w-full text-left
...
function Sidebar() {
  ...
  if (!theme || !theme.colors || !theme.colors.sidebar) {
    console.warn('Theme não está disponível ou incompleto no Sidebar');
    return (
