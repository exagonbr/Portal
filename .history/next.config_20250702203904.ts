...
const ContentSecurityPolicy: string = `
  ...
  font-src 'self' data:;
  connect-src 'self' http://localhost:3000 *.sabercon.com.br ws: wss:;
  media-src 'self' blob:;
  object-src 'none';
  base-uri 'self';
  form-action 'self';
...
const nextConfig: NextConfig = {
  ...
  // A função de rewrites foi desabilitada para centralizar o proxy na rota de API
  // em src/app/api/[...slug]/route.ts, que oferece mais controle.
  // async rewrites() {
  //   return [
  //     {
  //       source: '/api/:path*',
