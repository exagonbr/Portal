...
async function getAuthToken(): Promise<string | null> {
  try {
    console.log('🔑 Autenticando diretamente no backend...');
    const response = await axios.post('http://localhost:3001/api/auth/optimized/login', credentials);
...
function findApiRoutes(dir: string, prefix = ''): string[] {
    ...
    for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
            ...
            routes = routes.concat(findApiRoutes(fullPath, newPrefix));
...
async function main() {
  ...
  console.log('\n🚀 Iniciando teste de rota única...');
  await testRoute(token, '/auth/validate');
  console.log('\n\n🏁 Teste concluído.');
}
