...
async function main(): Promise<void> {
  ...
  log(`🌐 Abra o relatório HTML para visualização detalhada: ${htmlReportPath}`, 'cyan');
}

// Executar script
if (import.meta.url === `file://${process.argv[1]}`) {
  main().catch(error => {
    log(`💥 Erro fatal: ${error instanceof Error ? error.message : 'Erro desconhecido'}`, 'red');
    console.error(error);
    process.exit(1);
  });
}

export { discoverRoutes, analyzeRouteFile, makeRequest, authenticate, testRoute };