...
export const validateSendNotification = [
    ...
    .custom((emails) => {
      if (emails && emails.length > 0) {
        ...
        if (invalidEmails.length > 0) {
          throw new Error(`Emails inválidos encontrados: ${invalidEmails.join(', ')}`);
        }
      }
      return true;
    }),
  ...
  body()
    .custom((body) => {
      ...
      // Se sendEmail está ativo, deve ter pelo menos um destinatário
      if (body.sendEmail) {
        const hasRecipients = (
          (body.recipients?.emails && body.recipients.emails.length > 0) ||
          (body.recipients?.userIds && body.recipients.userIds.length > 0) ||
          (body.recipients?.roles && body.recipients.roles.length > 0)
        );
        
        if (!hasRecipients) {
          throw new Error('Para envio por email, é necessário especificar pelo menos um destinatário');
        }
      }
...
export const checkNotificationPermissions = (req: Request, res: Response, next: NextFunction): void => {
...
export const checkSentNotificationsPermissions = (req: Request, res: Response, next: NextFunction): void => {
  const user = req.user as any;
