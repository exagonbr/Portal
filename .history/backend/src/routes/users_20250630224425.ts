...
import { validateJWTSmart } from '../middleware/auth';
 ...
 */
router.get('/search', validateJWTSmart, async (req, res): Promise<void> => {
  try {
 ...
 */
router.get('/role/:roleId', validateJWTSmart, async (req, res) => {
  try {
 ...
 *         description: Users found
 */
router.get('/institution/:institutionId', validateJWTSmart, async (req, res) => {
  try {
    const { institutionId } = req.params;

    console.log('🔍 [USERS/BY-INSTITUTION] Buscando usuários por instituição:', institutionId);

    const users = await usersService.getUsersByInstitution(Number(institutionId));
