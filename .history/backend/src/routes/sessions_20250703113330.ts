...
type AuthenticatedRequestHandler = RequestHandler<any, any, any, any, { user?: Users; sessionId?: string }>;
...
const logoutAllHandler: AuthenticatedRequestHandler = async (req, res) => {
  try {
    ...
    if (req.user?.id) {
      try {
        removedSessions = await SessionService.destroyAllUserSessions(String(req.user.id));
      } catch (error) {
