...
// Estendendo o tipo Request para incluir sessionId e user
...
const logoutHandler: RequestHandler = async (req: AuthenticatedRequest, res) => {
  try {
...
const logoutAllHandler: RequestHandler = async (req: AuthenticatedRequest, res) => {
  try {
    let removedSessions = 0;
    
    if (req.user?.id) {
      try {
        removedSessions = await SessionService.destroyAllUserSessions(req.user.id);
      } catch (error) {
