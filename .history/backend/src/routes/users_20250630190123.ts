 ...
 */
router.post('/', usersAdminCorsMiddleware, authMiddleware, requireRole(['admin', 'SYSTEM_ADMIN', 'INSTITUTION_MANAGER', 'SCHOOL_MANAGER', 'INSTITUTION_MANAGER', 'manager']), async (req, res) => {
  try {
 ...
 *         description: User not found
 */
router.put('/:id', usersAdminCorsMiddleware, authMiddleware, requireRole(['admin', 'SYSTEM_ADMIN', 'INSTITUTION_MANAGER', 'SCHOOL_MANAGER', 'INSTITUTION_MANAGER', 'manager']), async (req, res) => {
  try {
    const { id } = req.params;
