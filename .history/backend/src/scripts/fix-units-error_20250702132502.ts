...
async function testUnitService(): Promise<boolean> {
  console.log('\n🧪 Testando UnitService...');
  
  try {
    // Verificar se o serviço pode ser importado
    console.log('📦 Importando UnitService...');
    const { UnitService } = await import('../services/UnitService');
    console.log('✅ UnitService importado com sucesso');
    ...
    console.log('🔍 Testando findUnitsWithFilters...');
    const result = await unitService.findUnitsWithFilters({
      page: 1,
      limit: 5
    });

    if (result.success) {
      console.log('✅ UnitService funcionando corretamente');
      console.log(`📊 Encontradas ${result.data?.units?.length || 0} unidades`);
      
      // Mostrar detalhes das unidades encontradas
      if (result.data?.units && result.data.units.length > 0) {
        console.log('📋 Unidades encontradas:');
        result.data.units.forEach((unit: any, index: number) => {
      ...
      console.log('❌ UnitService retornou erro:', result.error);
      return false;
    }

  } catch (error: any) {
    console.error('❌ Erro ao testar UnitService:', error.message);
...
async function main(): Promise<void> {
  ...
  try {
    ...
    // 2. Testar o serviço
    const serviceOk = await testUnitService();
    ...
    if (!serviceOk) {
      ...
      if (fixed) {
        ...
        const retestOk = await testUnitService();
...
export { checkUnitsTable, testUnitService, fixCommonIssues }; 