...
export class UserRepository extends BaseRepository<User> {
  ...
  async findUsersWithoutPassword(options?: { filters?: Partial<User>, orderBy?: { field: string, direction: 'asc' | 'desc' }, limit?: number }): Promise<UserWithoutPassword[]> {
    ...
    if (options?.orderBy) {
      ...
      const realField = fieldMapping[options.orderBy.field] || options.orderBy.field;
      query = query.orderBy(realField, options.orderBy.direction);
    }
  ...
  async getRecentLogins(limit: number = 10): Promise<any[]> {
    // This method will fail as 'last_login' does not exist on the User entity.
    // Returning an empty array to avoid crashes until the schema is updated.
    console.warn("UserRepository.getRecentLogins: 'last_login' column does not exist. Returning empty array.");
    return [];
  }
