...
import { RolePermissions } from '../../../src/types/roles';
...
export class GroupService {
  ...
  // Validações
  private validateGroupData(data: CreateUserGroupData | UpdateUserGroupData): void {
    ...
    if ('color' in data && data.color && !/^#[0-9A-F]{6}$/i.test(data.color)) {
      throw new Error('Cor deve estar no formato hexadecimal válido (ex: #FF5733)');
    }
  }
