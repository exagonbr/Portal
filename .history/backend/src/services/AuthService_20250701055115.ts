...
class AuthService {
  ...
  public async login(email: string, password: string): Promise<{
    ...
    try {
      ...
      // 3. Validar senha (assumindo que existe um método validatePassword)
      ...
      const accessToken = this.generateAccessToken(user, role, sessionId);
  ...
  public async refresh(refreshToken: string): Promise<{
    ...
    try {
      ...
      const newAccessToken = this.generateAccessToken(user, role, decoded.sessionId);
