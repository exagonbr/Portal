...
export class UsersService {
  ...
  async softDeleteUser(id: number): Promise<Users | null> {
    try {
      console.log('🗑️ [UsersService] Realizando soft delete do usuário:', id);
      const updatedUser = await this.usersRepository.updateUser(id, { deleted: true });
      if (updatedUser) {
  ...
  async changeUserPassword(id: number, currentPassword: any, newPassword: any): Promise<boolean> {
    try {
  ...
  async lockUserAccount(id: number): Promise<Users | null> {
    try {
      console.log('🔒 [UsersService] Bloqueando conta do usuário:', id);
      const updatedUser = await this.usersRepository.updateUser(id, { accountLocked: true });
      if (updatedUser) {
  ...
  async unlockUserAccount(id: number): Promise<Users | null> {
    try {
      console.log('🔓 [UsersService] Desbloqueando conta do usuário:', id);
      const updatedUser = await this.usersRepository.updateUser(id, { accountLocked: false });
      if (updatedUser) {
