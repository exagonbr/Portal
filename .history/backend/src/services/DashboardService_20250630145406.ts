...
export class DashboardService {
  ...
  static async getUserStats() {
    try {
      // Note: Active users metric is removed as 'last_login' field is not available.
      const [
        totalUsers,
        newThisMonth,
        byRole,
        ...
        this.userRepository.count({ created_at: { '>=': new Date(new Date().setDate(1)) } }),
      ...
      return {
        total: totalUsers,
        active: 0, // Placeholder for active users
        newThisMonth,
        byRole,
        byInstitution
      };
