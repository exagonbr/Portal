...
export class DashboardService {
  ...
  private static async getRecentActivity() {
    try {
      const [recentRegistrations, recentLogins] = await Promise.all([
        this.userRepository.findUsersWithoutPassword({ orderBy: { field: 'created_at', direction: 'desc' }, limit: 10 }),
        this.userRepository.getRecentLogins(10)
      ]);
  ...
  private static async getUserActivity(userId: string) {
    ...
    const user = await this.userRepository.findById(userId, ['created_at']);
