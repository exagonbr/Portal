...
export class OptimizedAuthService {
  ...
  static async getUserById(userId: string): Promise<UserWithRole | null> {
    try {
      ...
      // Determinar role e permissões baseado nos campos booleanos
      let roleInfo = { name: 'Estudante', slug: 'STUDENT' };
      let permissions: string[] = ['student', 'read'];
      ...
      return {
        ...
        email: user.email,
        name: user.full_name,
        role_id: user.is_admin ? 'admin' : (user.is_teacher ? 'teacher' : 'student'),
        institution_id: user.institution_id,
        is_active: user.enabled,
        created_at: user.date_created,
        updated_at: user.last_updated,
  ...
  static async hasPermission(userId: string, permission: string): Promise<boolean> {
    try {
      ...
      if (user.is_admin) {
        return ['admin', 'read', 'write', 'delete'].includes(permission);
      } else if (user.is_teacher) {
        return ['teacher', 'read', 'write'].includes(permission);
      } else if (user.is_student) {
        return ['student', 'read'].includes(permission);
      }
