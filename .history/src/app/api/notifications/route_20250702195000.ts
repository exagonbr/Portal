...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!session || !session.user || !session.user.id) {
      ...
      return NextResponse.json({ error: 'Não autorizado' }, { status: 401, headers: getCorsHeaders(request.headers.get('origin') || undefined) });
    }
    console.log("Auth success for user:", session.user.id);

    const userId = session.user.id;
    const userNotificationIds = mockUserNotifications.get(userId) || [];
    console.log(`Found ${userNotificationIds.length} notification IDs for user.`);

    let notifications = userNotificationIds.map((notifId: string) => {
      const notification = mockNotifications.get(notifId);
      if (!notification) return null;
      const userStatus = notification.user_statuses?.[userId] || { read: false, read_at: null, archived: false, acknowledged: false };
      return { ...notification, ...userStatus };
    }).filter(Boolean);
    console.log(`Mapped to ${notifications.length} full notification objects.`);

    // ... (o resto da lógica de filtro, sort e paginação permanece a mesma)
    // Se o erro persistir, logs adicionais podem ser inseridos aqui.
    ...
    console.log("--- End GET /notifications (Success) ---");
