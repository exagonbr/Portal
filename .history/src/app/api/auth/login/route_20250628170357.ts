...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    } else if (data.id && data.email) { // Apenas checar por id e email
      // O próprio objeto pode ser o usuário
      userData = data;
      
      // Normalizar o campo 'role'
      if (!userData.role && (userData.role_name || userData.role_slug)) {
        console.log(`Normalizando 'role': usando 'role_name' ou 'role_slug'`);
