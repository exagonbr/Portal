...
import { requireAuth, requirePermission, AuthSession } from '@/middleware/auth';
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  return requirePermission(['assignments.edit'])(async (req: NextRequest, auth: AuthSession) => {
    try {
      const { id } = params;
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  return requirePermission(['assignments.delete'])(async (req: NextRequest, auth: AuthSession) => {
    try {
      const { id } = params;
