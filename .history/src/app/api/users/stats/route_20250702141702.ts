...
export async function GET(request: NextRequest) {
  try {
    ...
    // Gerar um token válido para o backend usando os dados da sessão
    const backendToken = jwt.sign(
      ...
      JWT_CONFIG.JWT_SECRET || JWT_CONFIG.SECRET,
      { expiresIn: '5m' } // Token de curta duração para requisição interna
    );

    console.log('🔑 [/api/users/stats] Token gerado para backend');
    ...
    if (!response.ok) {
      ...
      console.log('❌ [/api/users/stats] Erro do backend:', errorText);
