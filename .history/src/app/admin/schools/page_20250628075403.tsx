    ...
    try {
      ...
      if (response && typeof response === 'object') {
        if (Array.isArray(response)) {
          ...
          // Resposta tem formato { items: [...] }
          unitsArray = response.items;
        } else if (response.data && Array.isArray(response.data)) {
          // Resposta tem formato { data: [...] }
          unitsArray = response.data;
        } else if (response.data && response.data.items && Array.isArray(response.data.items)) {
          // Resposta tem formato { data: { items: [...] } }
          unitsArray = response.data.items;
        } else {
      ...
      console.log('📋 Array de unidades extraído:', unitsArray);
      
      // Converter unidades para o formato esperado pelo frontend
      const unitsData = unitsArray.map(unit => {
