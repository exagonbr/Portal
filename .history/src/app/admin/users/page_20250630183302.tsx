  ...
  const loadUsers = useCallback(async (showLoadingIndicator = true) => {
    ...
    try {
      ...
      console.log('📥 Resposta recebida do usersService:', {
        ...
        primeiroUsuario: response.data?.[0]?.fullName || 'Nenhum',
        ultimoUsuario: response.data?.[response.data?.length - 1]?.fullName || 'Nenhum'
      });
      ...
      if (response.data && response.data.length > 0) {
        console.log(`✅ ${response.data.length} usuários carregados com sucesso (página ${response.pagination?.page} de ${response.pagination?.totalPages})`);
      } else {
