  ...
  const loadUsers = useCallback(async (showLoadingIndicator = true) => {
    ...
    try {
      ...
      console.log('📥 Resposta recebida do usersService:', {
        items: response.users?.length || 0,
        pagination: response.pagination,
        primeiroUsuario: response.users?.[0]?.fullName || 'Nenhum',
        ultimoUsuario: response.users?.[response.users?.length - 1]?.fullName || 'Nenhum'
      });
      ...
      if (response.users && response.users.length > 0) {
        console.log(`✅ ${response.users.length} usuários carregados com sucesso (página ${response.pagination?.page} de ${response.pagination?.totalPages})`);
      } else {
