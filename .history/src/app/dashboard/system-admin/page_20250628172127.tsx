...
function SystemAdminDashboardContent() {
  ...
  const loadRealUserStats = async () => {
    ...
    while (attempt <= maxRetries) {
      try {
        console.log(`📊 [DASHBOARD] Tentativa ${attempt + 1}/${maxRetries + 1} para carregar stats de usuários...`);
        
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), 65000); // 65s timeout (maior que o backend)
        ...
        if (response.ok) {
          ...
          if (result.success && result.data) {
            console.log('✅ [DASHBOARD] Stats de usuários carregadas com sucesso');
        ...
        if (attempt > maxRetries) {
          console.error('❌ [DASHBOARD] Todas as tentativas falharam, usando dados de fallback');
          
          // Usar dados de fallback após esgotar tentativas
          setRealUserStats({
            total_users: 18742,
            active_users: 15234,
            inactive_users: 3508,
        ...
        console.log(`⏳ [DASHBOARD] Aguardando ${delay}ms antes da próxima tentativa...`);
  ...
  return (
    <div className="p-3 max-w-7xl mx-auto">
      ...
      <div className="mb-6">
        ...
        {/* Resumo Geral do Sistema - sempre exibido */}
        <div className="mt-4">
          <ContentCard
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              ...
              <div className="text-center p-4 bg-blue-50 rounded-lg">
                <p className="text-2xl font-bold text-blue-600">
                  {realUserStats?.total_users?.toLocaleString('pt-BR') ?? '—'}
                </p>
                <p className="text-sm font-medium text-gray-700 mb-1">Total de Usuários</p>
                <p className="text-xs text-gray-500">
                  {realUserStats?.active_users?.toLocaleString('pt-BR') ?? '—'} ativos
                </p>
              </div>
              ...
              <div className="text-center p-4 bg-green-50 rounded-lg">
                <p className="text-2xl font-bold text-green-600">
                  {institutionStats?.totalInstitutions ?? institutions?.length ?? 0}
                </p>
                <p className="text-sm font-medium text-gray-700 mb-1">Instituições</p>
                <p className="text-xs text-gray-500">
                  {institutionStats?.activeInstitutions ?? institutions?.filter(i => i.active !== false).length ?? 0} ativas
                </p>
              </div>
              ...
              <div className="text-center p-4 bg-purple-50 rounded-lg">
                <p className="text-2xl font-bold text-purple-600">
                  {realUserStats?.recent_registrations ?? '—'}
                </p>
                <p className="text-sm font-medium text-gray-700 mb-1">Novos este Mês</p>
                <p className="text-xs text-gray-500">Registros recentes</p>
              </div>

              {/* Status do Sistema */}
              <div className="text-center p-4 bg-emerald-50 rounded-lg">
                <p className="text-2xl font-bold text-emerald-600">
                  {dashboardData?.system ? 'Online' : '—'}
                </p>
                <p className="text-sm font-medium text-gray-700 mb-1">Status Sistema</p>
                <p className="text-xs text-gray-500">
                  {dashboardData?.system ? formatUptime(dashboardData.system.uptime) : 'Carregando...'}
                </p>
              </div>
            </div>
          </ContentCard>
      ...
      {/* Primeira Linha - 3 Colunas: Usuários por Função, Crescimento de Usuários, Atividade por Hora */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 mb-3">
            
            {/* Usuários por Função - Dados Reais do Backend */}
            {usersByRoleData && (
              <div className="bg-white dark:bg-gray-100 rounded-lg shadow-md p-3">
                ...
                <div className="mt-3 grid grid-cols-2 gap-2">
                  {Object.entries(realUsersByRole && Object.keys(realUsersByRole).length > 0 ? realUsersByRole : realUserStats?.users_by_role || {}).map(([role, count], index) => {
                    const roleNames: Record<string, string> = {
                      'STUDENT': 'Alunos',
                      'TEACHER': 'Professores',
                    ...
                    const usersByRoleData = realUsersByRole && Object.keys(realUsersByRole).length > 0 ? realUsersByRole : realUserStats?.users_by_role || {};
      ...
      {/* Segunda Linha - Sessões por Dispositivo (largura completa) */}
      {sessionsByDeviceData && (
        <div className="bg-white dark:bg-gray-100 rounded-lg shadow-md p-3 mb-3">
                <div className="flex items-center justify-between mb-2">
                  <h3 className="text-base font-semibold flex items-center gap-2">
      ...
      {/* Terceira Linha - 2 Colunas: Instituições Ativas e Engajamento dos Usuários */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 mb-3">
        ...
        >
          <div className="space-y-2 max-h-64 overflow-y-auto">
            {institutions.slice(0, 8).map((institution, index) => (
              <SimpleCard key={institution.id || index} className="p-2" hover={false}>
                <div className="flex items-center justify-between">
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500">
                      {institution.users_count || 0} usuários • {institution.schools_count || 0} escolas
                    </p>
                  </div>
                  <div className="text-right ml-2">
                    <p className="text-xs font-bold text-green-600">
                      {institution.active !== false ? 'Ativa' : 'Inativa'}
                    </p>
                    <p className="text-xs text-gray-600">
                      {institution.users_count || 0} usuários
                    </p>
                  </div>
                </div>
              </SimpleCard>
            ...
            {institutionStats && (
              <div className="mt-3 pt-2 border-t border-gray-200">
                <div className="grid grid-cols-2 gap-2 text-center">
                  <div>
                    <p className="text-lg font-bold text-blue-600">{institutionStats.totalInstitutions}</p>
                    <p className="text-xs text-gray-600">Total</p>
                  </div>
                  <div>
                    <p className="text-lg font-bold text-green-600">{institutionStats.activeInstitutions}</p>
                    <p className="text-xs text-gray-600">Ativas</p>
                  </div>
                  <div>
                    <p className="text-lg font-bold text-purple-600">{institutionStats.totalUsers}</p>
                    <p className="text-xs text-gray-600">Usuários</p>
                  </div>
                  <div>
                    <p className="text-lg font-bold text-orange-600">{institutionStats.averageUsersPerInstitution}</p>
          ...
          >
            <div className="space-y-3">
              <div className="grid grid-cols-1 gap-2">
                ...
                <SimpleCard className="p-2" hover={false}>
                  <div className="flex justify-between items-center">
                    <span className="text-xs text-gray-600">Páginas por Sessão:</span>
                    <span className="font-bold text-purple-600 text-sm">{engagementMetrics.pagesPerSession}</span>
                  </div>
                </SimpleCard>
                <SimpleCard className="p-2" hover={false}>
                  <div className="flex justify-between items-center">
                    <span className="text-xs text-gray-600">Taxa de Bounce:</span>
                    <span className="font-bold text-red-600 text-sm">{engagementMetrics.bounceRate}%</span>
                  </div>
                </SimpleCard>
              </div>
            </div>
      ...
      {/* Quarta Linha - 3 Colunas: Infraestrutura AWS, Ações do Sistema, Acesso Rápido */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 mb-3">
        {/* Infraestrutura AWS */}
        {(dashboardData?.infrastructure?.aws || awsStats) && (
            ...
            <div className="space-y-2">
              {dashboardData?.infrastructure?.aws ? (
                <>
                  ...
                  <div className="flex justify-between">
                    <span className="text-xs text-gray-600">Latência:</span>
                    <span className="font-semibold text-sm text-blue-600">
                      {dashboardData?.infrastructure?.aws?.performance?.responseTime}ms
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-xs text-gray-600">Serviços:</span>
                    <span className="font-semibold text-sm text-purple-600">
                      {dashboardData?.infrastructure?.aws?.services?.length || 0}
                    </span>
                  </div>
                </>
              ) : awsStats ? (
                <>
                  <div className="flex justify-between">
                    ...
                    <span className={`font-semibold text-sm ${
                      ...
                      awsStats.success_rate >= 90 ? 'text-accent-yellow' : 'text-red-600'
                    }`}>
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold text-sm text-blue-600">
                      {awsStats.average_response_time}ms
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-xs text-gray-600">Conexões:</span>
                    <span className="font-semibold text-sm text-purple-600">
                      {awsStats.total_connections}
                    </span>
                  </div>
                </>
              ) : (
                <div className="text-center py-2">
                  ...
                  <p className="text-xs text-gray-500">AWS não configurado</p>
                </div>
              )}
              <button 
          ...
          {institutionDistributionData && (
            <div className="bg-white dark:bg-gray-100 rounded-lg shadow-md p-3 mb-2">
              <div className="flex items-center justify-between mb-2">
                <h3 className="text-base font-semibold flex items-center gap-2">
        ...
        {/* Ações do Sistema */}
        <div className="bg-white dark:bg-gray-100 rounded-lg shadow-md p-3">
          <h3 className="text-base font-semibold mb-2">Ações do Sistema</h3>
          <div className="space-y-2">
            <button 
              onClick={() => router.push('/admin/institutions')}
        ...
        {/* Acesso Rápido */}
        <div className="bg-white dark:bg-gray-100 rounded-lg shadow-md p-3">
          <h3 className="text-base font-semibold mb-2">Acesso Rápido</h3>
