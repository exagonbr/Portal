...
function ProtectedRouteContent({
  ...
  // Hook seguro para o tema - pode não estar disponível durante SSR/hidratação
  let theme;
  try {
    const themeContext = useTheme();
    theme = themeContext.theme;
  } catch (error) {
    // Fallback para quando ThemeProvider não está disponível ainda
    theme = {
      colors: {
        background: { primary: '#ffffff', card: '#f8f9fa' },
        primary: { DEFAULT: '#0f3460', dark: '#0a2847', contrast: '#ffffff' },
