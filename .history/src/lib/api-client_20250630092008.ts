...
class ApiClient {
  ...
  private async makeRequest<T>(
    ...
    try {
      ...
      console.log(`[API-CLIENT] Tentando requisição para: ${endpoint}`);

      if (isFirefox()) {
        response = FirefoxUtils.safeFetch
          ? await FirefoxUtils.safeFetch(endpoint, options)
      ...
      if (!response.ok) {
        if (response.status === 500) {
          logHttp500Error(endpoint, options.method || 'GET', '', response.status, data, responseText, options.headers as Record<string, string>, Object.fromEntries(response.headers.entries()));
        }
        throw {
      ...
      if (isNetworkError) {
        console.warn(`[API-CLIENT] API primária falhou. Tentando fallback...`);
        return this.makeRequest<T>(this.buildURL(endpoint, { method: 'GET' }));
      }
