...
function checkTokenStatus(result: AuthDiagnosticResult): void {
  try {
    ...
    if (debugInfo.tokenInfo?.exp) {
      const now = Math.floor(Date.now() / 1000);
      if (debugInfo.tokenInfo.exp < now) {
      ...
      const timeUntilExpiry = debugInfo.tokenInfo.exp - now;
    ...
    // Se chegou aqui, o token existe e não está expirado
...
function checkStorageStatus(result: AuthDiagnosticResult): void {
  try {
    ...
    if (debugInfo.hasAccessToken) storageItems++;
    if (debugInfo.hasRefreshToken) storageItems++;
    if (debugInfo.hasUserData) storageItems++;
    ...
    // Verificar cookies
    if (debugInfo.cookies && debugInfo.cookies.length > 0) {
      console.log(`🍪 Cookies de autenticação encontrados: ${debugInfo.cookies.length}`);
    }
...
/**
 * Executa diagnóstico de autenticação (alias para debugAuth)
 */
