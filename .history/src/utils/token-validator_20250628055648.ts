...
export async function syncTokenWithApiClient(token?: string): Promise<boolean> {
  try {
    ...
    try {
      // Importar dinamicamente o apiClient com tratamento de erro
      const chunkRetryModule = await import('./chunk-retry');
      
      if (!chunkRetryModule || !chunkRetryModule.importApiClient) {
        console.error('❌ [TOKEN-VALIDATOR] Módulo chunk-retry não carregou corretamente');
        return false;
      }

      const apiClientModule = await chunkRetryModule.importApiClient();
      
      if (apiClientModule?.apiClient && typeof apiClientModule.apiClient.setAuthToken === 'function') {
        apiClientModule.apiClient.setAuthToken(tokenToSync);
      ...
      console.error('❌ [TOKEN-VALIDATOR] Erro ao importar módulos:', importError);
