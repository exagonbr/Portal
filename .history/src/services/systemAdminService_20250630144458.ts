  ...
  async getRealUserStats(): Promise<any> {
    return withAutoRefresh(async () => {
      try {
        ...
        // Em vez de lançar um erro, registrar e retornar fallback para maior resiliência
        console.error(`❌ [REAL-USER-STATS] Falha na API: ${response.message || 'Erro desconhecido'}`);
        return this.getFallbackUserStats();
      } catch (error: unknown) {
