...
class SystemAdminService {
  ...
  async getUsersByRole(): Promise<Record<string, number>> {
    try {
      ...
      throw new Error(response?.message || 'Falha ao carregar dados de usuários da API.');
    } catch (error: unknown) {
      console.error('❌ [SYSTEM-ADMIN-SERVICE] Erro em getUsersByRole, usando fallback:', error);
