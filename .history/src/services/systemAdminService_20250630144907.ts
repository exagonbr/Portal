  ...
  async getUsersByRole(): Promise<Record<string, number>> {
    try {
      ...
      if (!authStatus.authenticated) {
        const refreshSuccess = await autoRefreshToken();
        if (!refreshSuccess) {
          console.error('❌ [SYSTEM-ADMIN-SERVICE] Falha no refresh em getUsersByRole, usando fallback.');
      ...
      console.error(`❌ [SYSTEM-ADMIN-SERVICE] Falha na API em getUsersByRole: ${response?.message || 'Erro desconhecido'}`);
      return this.getFallbackUserStats().users_by_role;
    } catch (error: unknown) {
      console.error('❌ [SYSTEM-ADMIN-SERVICE] Erro inesperado em getUsersByRole, usando fallback:', error);
      return this.getFallbackUserStats().users_by_role;
    }
  }
  ...
  async getRealUserStats(): Promise<any> {
    try {
      ...
      if (!authStatus.authenticated) {
        const refreshSuccess = await autoRefreshToken();
        if (!refreshSuccess) {
          console.error('❌ [REAL-USER-STATS] Falha no refresh em getRealUserStats, usando fallback.');
      ...
      console.error(`❌ [REAL-USER-STATS] Falha na API em getRealUserStats: ${response?.message || 'Erro desconhecido'}`);
      return this.getFallbackUserStats();
    } catch (error: unknown) {
      console.error('❌ [REAL-USER-STATS] Erro inesperado em getRealUserStats, usando fallback:', error);
      return this.getFallbackUserStats();
    }
  }
}
