...
class SystemAdminService {
  ...
  async getRealUserStats(): Promise<any> {
    try {
      ...
      throw new Error(response?.message || 'Falha ao carregar estatísticas de usuários da API.');
    } catch (error: unknown) {
      console.error('❌ [REAL-USER-STATS] Erro em getRealUserStats, usando fallback:', error);
