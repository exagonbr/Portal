    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        const isRetryableError = error && typeof error === 'object' &&
          ('status' in error && [502, 503, 504].includes(error.status as number)) ||
          ('message' in error && typeof error.message === 'string' &&
           (error.message.includes('504') || error.message.includes('Gateway Time-out') || error.message.includes('timeout')));
