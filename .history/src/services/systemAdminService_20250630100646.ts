  ...
  async getRoleStats(): Promise<any> {
    return withAutoRefresh(async () => {
      try {
        ...
        const currentToken = getCurrentToken();
        if (!currentToken) {
          console.warn('⚠️ [ROLE-STATS] Nenhum token disponível, fazendo fallback');
          return null;
        }

        // Validar token antes da requisição
        const tokenValidation = validateToken(currentToken);
        if (!tokenValidation.isValid) {
          console.warn('⚠️ [ROLE-STATS] Token inválido, tentando refresh automático');
          const refreshSuccess = await autoRefreshToken();
          if (!refreshSuccess) {
            console.error('❌ [ROLE-STATS] Falha no refresh do token, retornando null');
            return null;
          }
        }
        ...
        if (error && typeof error === 'object' && 'status' in error && error.status === 401) {
          console.log('🔄 [ROLE-STATS] Erro 401 detectado, limpando tokens');
          clearAllTokens();
          
          // Redirecionar para login se estivermos no browser
          if (typeof window !== 'undefined') {
            console.log('🔄 [ROLE-STATS] Redirecionando para login...');
  ...
  async getAwsConnectionStats(): Promise<any> {
    return withAutoRefresh(async () => {
      try {
        ...
        const currentToken = getCurrentToken();
        if (!currentToken) {
          console.warn('⚠️ [AWS-STATS] Nenhum token disponível, fazendo fallback');
          return null;
        }
        ...
        const tokenValidation = validateToken(currentToken);
        if (!tokenValidation.isValid) {
          console.warn('⚠️ [AWS-STATS] Token inválido, tentando refresh automático');
          const refreshSuccess = await autoRefreshToken();
          if (!refreshSuccess) {
            console.error('❌ [AWS-STATS] Falha no refresh do token, retornando null');
            return null;
          }
        }
        ...
        // Verificar se é erro de autenticação
        if (error && typeof error === 'object' && 'status' in error && error.status === 401) {
          console.log('🔄 [AWS-STATS] Erro 401 detectado, limpando tokens');
          clearAllTokens();
          
          // Redirecionar para login se estivermos no browser
          if (typeof window !== 'undefined') {
            console.log('🔄 [AWS-STATS] Redirecionando para login...');
            window.location.href = '/login';
          }
        }
  ...
  async getRealUserStats(): Promise<any> {
    return withAutoRefresh(async () => {
      try {
        ...
        const currentToken = getCurrentToken();
        if (!currentToken) {
          console.warn('⚠️ [REAL-USER-STATS] Nenhum token disponível, fazendo fallback');
          return {
            total_users: 18742,
            active_users: 15234,
            inactive_users: 3508,
        ...
        const tokenValidation = validateToken(currentToken);
        if (!tokenValidation.isValid) {
          console.warn('⚠️ [REAL-USER-STATS] Token inválido, tentando refresh automático');
          const refreshSuccess = await autoRefreshToken();
          if (!refreshSuccess) {
            console.error('❌ [REAL-USER-STATS] Falha no refresh do token, usando fallback');
            return {
              total_users: 18742,
              active_users: 15234,
              inactive_users: 3508,
        ...
        // Verificar se é erro de autenticação
        if (error && typeof error === 'object' && 'status' in error && error.status === 401) {
          console.log('🔄 [REAL-USER-STATS] Erro 401 detectado, limpando tokens e usando fallback');
          clearAllTokens();
          
          // Redirecionar para login se estivermos no browser
          if (typeof window !== 'undefined') {
            console.log('🔄 [REAL-USER-STATS] Redirecionando para login...');
            window.location.href = '/login';
            return {};
          }
        }
