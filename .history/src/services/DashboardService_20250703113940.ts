...
export const dashboardService = {
  ...
  async getUserStats(institutionId?: string): Promise<{ total: number; active: number; newToday: number }> {
    try {
      const totalUsers = await UserRepository.count(institutionId ? { institutionId } : {})
  ...
  async getAnalytics(type: 'users' | 'sessions' | 'activities', period: 'day' | 'week' | 'month'): Promise<AnalyticsData> {
    try {
