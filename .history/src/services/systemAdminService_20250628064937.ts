  ...
  async getRealTimeMetrics(): Promise<RealTimeMetrics> {
    try {
      ...
      if (error?.message?.includes('Token') || 
        ...
        try {
          const chunkRetryModule = await import('../utils/chunk-retry');
          
          if (chunkRetryModule && typeof chunkRetryModule.importApiClient === 'function') {
            const apiClientModule = await chunkRetryModule.importApiClient();
            
            if (apiClientModule?.apiClient && typeof apiClientModule.apiClient.clearAuth === 'function') {
            ...
            console.warn('⚠️ [SYSTEM-ADMIN] Módulo chunk-retry não carregou corretamente');
          }
        } catch (clearError) {
