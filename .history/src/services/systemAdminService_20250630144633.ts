...
class SystemAdminService {
  ...
  async getUsersByRole(): Promise<Record<string, number>> {
    try {
      ...
      if (response && response.success && response.data?.data?.users_by_role) {
      ...
      console.error('❌ [SYSTEM-ADMIN-SERVICE] Erro na resposta da API:', errorMessage);
      
      // Se a resposta indicar erro de autenticação, lançar para ser tratado pelo catch
      ...
      // Para outros erros de API, não lançar exceção, apenas usar o fallback
      console.warn('⚠️ [SYSTEM-ADMIN-SERVICE] Usando dados de fallback devido a erro na API.');
  ...
  async getRealUserStats(): Promise<any> {
    return withAutoRefresh(async () => {
      try {
        ...
        const errorMessage = response?.message || 'Erro desconhecido na resposta da API';
        console.warn('⚠️ [REAL-USER-STATS] Resposta sem sucesso ou dados:', { response, errorMessage });
        ...
        // Para outros erros, registrar e usar fallback
        console.error(`❌ [REAL-USER-STATS] Falha na API: ${errorMessage}`);
        return this.getFallbackUserStats();
      } catch (error: unknown) {
