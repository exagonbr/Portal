...
const getBaseUrls = () => {
  if (isProduction) {
    return {
      ...
      INTERNAL_API_URL: process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001/api'
    };
  }
  ...
  return {
    FRONTEND_URL: 'http://localhost:3000',
    BACKEND_URL: '/api', // Para o cliente, ainda usa o proxy
    API_BASE_URL: '/api', // Para o cliente, ainda usa o proxy
    INTERNAL_API_URL: process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001/api'
  };
};
