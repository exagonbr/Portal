...
import { UserRole, hasPermission, ROLE_PERMISSIONS } from '@/types/roles';
...
export function useMenuPermissions() {
  ...
  const canAccessRoute = useMemo(() => {
    return (routePath: string, requiredPermission?: keyof typeof ROLE_PERMISSIONS[UserRole.SYSTEM_ADMIN]): boolean => {
  ...
  const hasAnyPermission = useMemo(() => {
    return (permissions: (keyof typeof ROLE_PERMISSIONS[UserRole.SYSTEM_ADMIN])[]): boolean => {
  ...
  return {
    ...
    canAccessRoute,
    getUserPermissions,
    hasPermission: (permission: keyof typeof ROLE_PERMISSIONS[UserRole.SYSTEM_ADMIN]) => hasPermission(userRole, permission),
    hasAnyPermission,
    hasAllPermissions,
  };
}
