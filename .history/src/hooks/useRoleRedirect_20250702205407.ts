...
export function useRoleRedirect(
  ...
  useEffect(() => {
    ...
    if (!isValidRole(userRole)) {
      ...
      const redirectUrl = fallbackPath.startsWith('/') && frontendUrl ? `${frontendUrl}${fallbackPath}` : fallbackPath;
    ...
    if (dashboardPath) {
      ...
      const frontendUrl = process.env.FRONTEND_URL || process.env.NEXTAUTH_URL || '';
      const fullDashboardPath = dashboardPath.startsWith('/') && frontendUrl ? `${frontendUrl}${dashboardPath}` : dashboardPath;
